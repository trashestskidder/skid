...
AttackFunc = function()
    local delay = math.random(0.1, 0.3) -- increase the attack delay to make it less suspicious
    task.wait(delay)
    AddAttack(BladeHits(65))
end

local Tick = tick()
local Delay = _G.FastAttackDelay -- increase the delay between attacks
RunAttack = function()
    if (tick() - Tick) >= math.clamp(Delay, 0.020, 1) then
...
task.spawn(function()
    while _G.FastAttack do
        ...
        local Bruh = require(game:GetService("Players").LocalPlayer.PlayerScripts.CombatFramework)
        local Gay = debug.getupvalues(Bruh)[2]
        if typeof(Gay) == "table" then
            Gay.activeController.timeToNextAttack = 1
            Gay.activeController.active = false
            Gay.activeController.timeToNextBlock = 0
            Gay.activeController.focusStart = 0
            Gay.activeController.increment = 4
            Gay.activeController.blocking = false
            Gay.activeController.attacking = false
            Gay.activeController.humanoid.AutoRotate = false
        end
    end
end)
