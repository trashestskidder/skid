...
local chooseTeamGui = mainGui:FindFirstChild("ChooseTeam")
_G.Team = "Pirate"
if chooseTeamGui then
	repeat
		...
		if chooseTeamGui.Visible then
			local teamButton
			if _G.Team == "Pirate" or _G.Team == nil then
				teamButton = chooseTeamGui.Container.Pirates.Frame.TextButton
			elseif _G.Team == "Marine" then
				teamButton = chooseTeamGui.Container.Marines.Frame.TextButton
			end
			if teamButton then
				for _, connection in ipairs(getconnections(teamButton.Activated)) do
					connection.Function()
				end
			end
		end
...
local function bringMobs()
    if BringMob then
        for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
            ...
            if not success then
                warn("Error handling mob:", errorMessage)
            end
        end
    end
end
...
local tickInterval = _G.FastAttackDelay
local lastTick = tick()

local function RunAttack()
    if (tick() - lastTick) >= tickInterval then
...
function CancelTween()
	if _G.currentTween then
	  ...
	  _G.currentTween = nil -- Clear the stored tween
	end
end
