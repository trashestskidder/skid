...
function LoadSettings()
    if readfile and writefile and isfile and isfolder then
        if not isfolder("Crazzy_Hub") then
            makefolder("Crazzy_Hub")
        end
        if not isfolder("Crazzy_Hub/Blox Fruits/") then
            makefolder("Crazzy_Hub/Blox Fruits/")
        end
        if not isfile("Crazzy_Hub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
            writefile("Crazzy_Hub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json",
                game:GetService("HttpService"):JSONEncode(_G.Settings))
        else
            local Decode = game:GetService("HttpService"):JSONDecode(readfile("Crazzy_Hub/Blox Fruits/" ..
...
function CurrentWeapon()
	...
	local val = ac.blades[1]
	if not val then
		return game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool").Name
	end
	pcall(function() 
		while val.Parent ~= game.Players.LocalPlayer.Character do
			val = val.Parent
		end 
	end)
	if not val then 
...
function getAllBladeHitsPlayers(val)
	local Hits = {}
	for i = 1, #game:GetService("Workspace").Characters:GetChildren() do 
		local Val = game:GetService("Workspace").Characters:GetChildren()[i] 
		local Val1 = Val:FindFirstChildOfClass("Humanoid")
		if Val.Name ~= game.Players.LocalPlayer.Name and Val1 and Val1.RootPart and Val1.Health > 0 and game.Players.LocalPlayer:DistanceFromCharacter(Val1.RootPart.Position) < val + 5 then
			table.insert(Hits, Val1.RootPart)
		end
	end
	return Hits
end

function getAllBladeHits(val)
	local Hits = {}
	for i = 1, #game:GetService("Workspace").Enemies:GetChildren() do 
		local Val = game:GetService("Workspace").Enemies:GetChildren()[i] 
		local Val1 = Val:FindFirstChildOfClass("Humanoid")
		if Val1 and Val1.RootPart and Val1.Health > 0 and game.Players.LocalPlayer:DistanceFromCharacter(Val1.RootPart.Position) < val + 5 then
			table.insert(Hits, Val1.RootPart)
		end
	end
	return Hits
...
function AttackFunc()
	if CombatFrameworkR.activeController and CombatFrameworkR.activeController.equipped then
		for indexincrement = 1, 1 do
			...
			if #bladehit > 0 then
				local AcAttack8 = debug.getupvalue(CombatFrameworkR.activeController.attack, 5)
				local AcAttack7 = debug.getupvalue(CombatFrameworkR.activeController.attack, 4)
				local AcAttack10 = debug.getupvalue(CombatFrameworkR.activeController.attack, 7)
				local NumberAc12 = (AcAttack8 * 798405 + AcAttack7 * 727595) % debug.getupvalue(CombatFrameworkR.activeController.attack, 6)
				local NumberAc13 = AcAttack7 * 798405
				(function()
					NumberAc12 = (NumberAc12 * debug.getupvalue(CombatFrameworkR.activeController.attack, 6) + NumberAc13) % 1099511627776
					AcAttack8 = math.floor(NumberAc12 / debug.getupvalue(CombatFrameworkR.activeController.attack, 6))
					AcAttack7 = NumberAc12 - AcAttack8 * debug.getupvalue(CombatFrameworkR.activeController.attack, 6)
				end)()
				AcAttack10 = AcAttack10 + 1
				debug.setupvalue(CombatFrameworkR.activeController.attack, 5, AcAttack8)
				debug.setupvalue(CombatFrameworkR.activeController.attack, 6, debug.getupvalue(CombatFrameworkR.activeController.attack, 6))
				debug.setupvalue(CombatFrameworkR.activeController.attack, 4, AcAttack7)
				debug.setupvalue(CombatFrameworkR.activeController.attack, 7, AcAttack10)
				for k, v in pairs(CombatFrameworkR.activeController.animator.anims.basic) do
...
local function getIsLand(...)
    ...
    if game.Players.LocalPlayer.Team then
        for _, v in pairs(game.Workspace._WorldOrigin.PlayerSpawns:FindFirstChild(tostring(game.Players.LocalPlayer.Team)):GetChildren()) do
...
local function toTarget(...)
    ...
    local tween, err = pcall(function()
        ...
        tween = tweenService:Create(game.Players.LocalPlayer.Character["HumanoidRootPart"], info, {
            CFrame = realTarget
        })
        tween:Play()
    end)
...
function TP(Pos)
	...
	pcall(function()
		tween = game:GetService("TweenService"):Create(game.Players.LocalPlayer.Character.HumanoidRootPart, TweenInfo.new(Distance / 210, Enum.EasingStyle.Linear), {
			CFrame = Pos
		})
	end)
	tween:Play()
...
game:GetService("StarterGui"):SetCore("SendNotification", {
	Title = "Crazzy Hub V3",
	Text = "Function Loadded Successfully",
	Icon = "rbxassetid://15689000757",
	Duration = 2
})
----------------------------------------------------------------------------------------------------------------------Info Tab
...
local ScriptStatus = Info:AddSection("Script Status")

local AutoFarmLevelStatus = Info:AddParagraph({"Auto Farm Level"})
local AutoFarmNearestStatus = Info:AddParagraph({"Auto Farm Nearest Monster"})
local AutoFarmChestStatus = Info:AddParagraph({"Auto Farm Chest"})
local AutoFarmChestFastStatus = Info:AddParagraph({"Auto Farm Chest Fast"})
spawn(function()
    while wait() do
        if _G.Settings.AutoFarm == true then
            AutoFarmLevelStatus:Set("Status : 🟢")
        else
            AutoFarmLevelStatus:Set("Status : 🔴")
        end
		if _G.Settings.NeareastFarm == true then
            AutoFarmNearestStatus:Set("Status : 🟢")
        else
            AutoFarmNearestStatus:Set("Status : 🔴")
        end
		if _G.Settings.AutoFarmChest == true then
            AutoFarmChestStatus:Set("Status : 🟢")
        else
            AutoFarmChestStatus:Set("Status : 🔴")
        end
		if _G.Settings.ChestBypassBypass == true then
            AutoFarmChestFastStatus:Set("Status : 🟢")
        else
            AutoFarmChestFastStatus:Set("Status : 🔴")
        end
    end
...
game:GetService("StarterGui"):SetCore("SendNotification", {
	Title = "Crazzy Hub V3",
	Text = "Script Loadded Successfully",
	Icon = "rbxassetid://15689000757",
	Duration = 2
})
